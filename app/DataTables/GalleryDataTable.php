...
class GalleryDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $dataTable = $dataTable
            ...
            ->editColumn('clinic.name', function ($gallery) {
                return getLinksColumnByRouteName([$gallery->clinic], 'clinics.edit', 'id', 'name');
            })
    ...
    protected function getColumns()
    {
        $columns = [
            ...
            [
                'data' => 'clinic.name',
                'name' => 'clinic.name',
                'title' => trans('lang.gallery_clinic_id'),
            ],
            [
                'data' => 'updated_at',
    ...
    public function query(Gallery $model)
    {
        if (auth()->user()->hasRole('clinic_owner')) {
            return $model->newQuery()->with("clinic")
                ->join('clinic_users', 'clinic_users.clinic_id', '=', 'galleries.clinic_id')
                ->groupBy('galleries.id')
                ->select("$model->table.*")
                ->where('clinic_users.user_id', auth()->id());
        }if (auth()->user()->hasRole('doctor')) {
        return $model->newQuery()->with("clinic")
            ->join('doctors', 'doctors.clinic_id', '=', 'galleries.clinic_id')
            ->groupBy('galleries.id')
            ->select("$model->table.*")
